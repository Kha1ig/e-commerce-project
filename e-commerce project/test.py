...
def shop(request):
    ...
    if price:
        ...
        shops=Shops.objects.all().filter(price_gte=values[0]).filter(price_lte=values[1])[per_count*(page-1):(page*per_count)]
        all_shops=Shops.objects.all().filter(price_gte=values[0]).filter(price_lte=values[1]).count()
        total_page_count = math.ceil( all_shops / per_count )
        page_range = range(1, total_page_count + 1)
    # Search for ORDER 
    ...
    if search:
        shops=Shops.objects.all().filter( Q(title_icontains=search) | Q(categorycategoryicontains=search) | Q(brandbrand_icontains=search))[per_count*(page-1):(page*per_count)]
        all_shops=Shops.objects.all().filter(Q(title_icontains=search) | Q(categorycategoryicontains=search) | Q(brandbrand_icontains=search)).count()
        total_page_count = math.ceil( all_shops / per_count )
        page_range = range(1, total_page_count + 1)
